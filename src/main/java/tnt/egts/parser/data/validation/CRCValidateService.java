...
public class CRCValidateService implements CRCValidate {
    ...
    public boolean CRC8Correct(byte[] income) {
        byte[] onlyHead = ArrayUtils.createSubArray(income, 0, income[ByteFixedPositions.HEAD_LENGTH_INDEX]);
        byte[] testIncome = ArrayUtils.createSubArray(income, 0, onlyHead.length - 1);
    ...
    public boolean CRC16Correct(byte[] income) {
        byte[] dataOnly = ArrayUtils.createSubArray(income,
                income[ByteFixedPositions.HEAD_LENGTH_INDEX], income.length - 1);
        byte[] dataCrcFree = ArrayUtils.createSubArray(dataOnly, 0, dataOnly.length - 2);
        byte[] dataCrc = ArrayUtils.createSubArray(dataOnly, dataOnly.length - 2, dataOnly.length);
